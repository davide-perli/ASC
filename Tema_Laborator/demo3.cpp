...
int no_total_blocks = 1024;
int memory[1024] = {0};
int block_size = 8;

// modific functia astfel incat sa apeleze get pentru toate fisierele existente dupa ce ele au fost adaugate (toate)
void et_add_file_to_memory(int id, int f_size)
{
    if(id < 1 || id > 255)
    {
        cout<<"id-ul trebuie sa fie intre 1 si 255" << endl;
        return;
    }
    ...
    if (total_allocated_blocks + no_blocks_file > no_total_blocks)
    {
        cout << "Nu este destula memorie pentru adaugare fisier " << id << " pe " << f_size << " KB" << endl;
        return;
    }
    ///////////////////////
    ...
    for(int i = 0; i < no_total_blocks; i++)
    {
        if(memory[i] == 0)// am gasit o secventa care incepe cu zero ocupand de cel putin un indice din memorie
        {
            ...
            if(free_count == no_blocks_file && next_available_block+free_count<=no_total_blocks)
            {
    ...
    if(next_available_block != -1 && next_available_block+no_blocks_file<=no_total_blocks)
    {
...
void et_get_all_positions()
{
    int last_id = -1;// ultimul id procesat
    for (int i = 0; i < no_total_blocks; i++)
    {
        if(memory[i] != 0 && memory[i] != last_id)
        {
            last_id = memory[i];// ultimul id va fi acum un alt descriptor
            cout<<last_id<<": ";
            et_get_file_positions(last_id);// apelam get cand intalnim un nou descriptor neafisat inca
        }
    }
}
...
{
    int aux_memory[1024];// memorie auxiliara pentru a stoca fisierele compact
...
{
    int operation, k;
    int file_descriptor, file_size;
    int num_operations;

    cout<<"Introduceti numarul de operatii: ";
    cin>>num_operations;

    for(int i = 0; i < num_operations; i++)
    {
        cout<<"Selectati operatia (1-add, 2-get, 3-delete, 4-defragmentation): ";
        cin>>operation;
        ...
        switch(operation)
        {
            ...
            case 3:
                cout << "id-ul fisierului care trebuie sters: ";
                cin >> file_descriptor;
            ...
            case 4:
                cout<<"Efectuez defragmentarea memoriei: "<<endl;
